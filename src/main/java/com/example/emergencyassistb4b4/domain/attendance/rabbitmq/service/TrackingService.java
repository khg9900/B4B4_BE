...
public class TrackingService {
    ...
    public void scheduleTrackingForTeam(Long teamId) {
        ...
        participants.forEach(p -> rabbitMQRedisService.mapVolunteerToUser(p.getId(), p.getUser().getId()));

        TrackingSessionDto sessionDto = TrackingSessionDto.from(post, team, location, policy, participantUserIds);

        LocalDateTime checkinStart = policy.getCheckinStart();

        // READY 메시지 예약 (출석 시작 1분 전)
    ...
    public void scheduleTrackingAtTime(MessageWrapper wrapper, LocalDateTime scheduledTime) {
        ...
        if (delayMillis < 0) {
            log.warn("예약 시간이 과거임, 즉시 발송: {}", scheduledTime);
            delayMillis = 0;
        }
